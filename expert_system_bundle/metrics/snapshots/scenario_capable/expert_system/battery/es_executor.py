"""
GPS, propulsion, communication, wind, and battery-capable Expert System executor.
"""
...
class ExpertSystemExecutor(Node):
    ...
    def __init__(self) -> None:
        ...
        self.get_logger().info(
            "Mode: GPS + PROPULSION + COMMUNICATION + WIND + BATTERY CAPABLE"
        )
        self.get_logger().info(
            "========================================"
        )
    ...
    def _tick_recovery(self) -> None:
        ...
        if self.recovery_action == "SAFE_LAND":
            ...
            self.get_logger().warning(
                "ES RECOVERY COMPLETED: SAFE_LAND"
            )
        ...
        if self.recovery_action in {
            ...
            self._publish_progress(
                state_name=recovery_action,
                task_type="recovery",
                status="COMPLETED",
            )

            self.get_logger().warning(
                f"ES RECOVERY COMPLETED: "
                f"{recovery_action}"
            )
