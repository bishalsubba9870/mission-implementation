"""FSM mission executor with GPS, propulsion, and communication recovery."""
...
class FSMExecutor(Node):
    ...
    def _build_fsm(self) -> None:
        """Build mission and runtime-recovery structure."""
        ...
        for index, state_name in enumerate(task_states):
            next_state = (
                task_states[index + 1]
            ...
            if self._is_navigation_state(state_name):
                self.transitions[
                    (
                        state_name,
                        'GPS_LOST',
    ...
    def _enter_state(
        ...
        """Execute entry behavior of one state."""

        if state_name == 'COMPLETED':
            self.get_logger().info(
                'STATE: COMPLETED'
            )
            self._finish_mission('SUCCESS')
            return
    ...
    def _timer_callback(self) -> None:
        """Complete currently active automatic state."""
        ...
        if self.current_state == 'RETURN_TO_HOME':
            self._complete_return_to_home()
            return
    ...
    def _complete_recovery_land(self) -> None:
    ...
    def _complete_return_to_home(self) -> None:
        """Complete return-to-home recovery."""
    ...
    def _handle_gps_available(self) -> None:
        """Resume mission after GPS recovery."""
    ...
    def _handle_communication_available(
        ...
        """Resume mission after communication recovery."""
    ...
    def _handle_communication_recovery_failed(
        ...
        """Handle failed communication recovery."""

        if self.current_state != 'COMMUNICATION_RECOVERY':
            self.get_logger().warning(
                'COMMUNICATION_RECOVERY_FAILED '
                'received outside '
                'COMMUNICATION_RECOVERY.'
    ...
    def _resume_interrupted_state(
        ...
        """Resume the interrupted mission state."""
