"""
GPS-, propulsion-, communication-, wind-, and battery-capable UAV HTN domain.

Normal mission execution and runtime recovery are represented
using primitive operators and HTN decomposition methods.
"""
...
@dataclass(frozen=True)
class Method:
    """Method for decomposing a compound HTN task."""
...
class HTNDomain:
    """Collection of operators and decomposition methods."""
...
def abort_mission_effect(
    ...
    state["mission_interrupted"] = True
...
def resume_effect(
    ...
    state["mission_interrupted"] = False

    state["gps_was_lost"] = False
    state["gps_recovery_failed"] = False

    state["communication_was_lost"] = False
    state["communication_recovery_failed"] = False
...
def build_uav_domain(
    ...
    """Build the current scenario-capable UAV HTN domain."""
